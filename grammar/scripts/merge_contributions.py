#!/usr/bin/env python3
"""
merge_contributions.py — Merge grammar contribution exports into the canonical data files.
...
def write_report(report_path: Path, contributions_processed: list[dict],
                 all_new_nodes: list[dict], all_new_edges: list[dict],
                 all_conflicts: list[dict]) -> None:
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    lines = [
        f"# Grammar Merge Report — {today}",
        "",
        "## Summary",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge grammar contribution exports into canonical grammar_nodes.json and grammar_edges.json"
    )
    ...
    parser.add_argument("--contributions", nargs="+", required=True, metavar="DIR",
                        help="One or more directories containing contribution exports "
                             "(each must contain manifest.json and contrib_grammar_nodes.json)")
    parser.add_argument("--output", default="data/", metavar="DIR",
                        help="Directory to write merged grammar files (default: data/)")
    ...
    print(f"Canonical state: {len(canonical_nodes)} nodes, {len(canonical_edges)} edges")
    print()
    ...
    for contrib_path_str in args.contributions:
        ...
        if node_conflicts:
            print(f"  Conflicts: {len(node_conflicts)} nodes")
        print()
    ...
    write_report(report_path, contributions_processed, all_new_nodes, all_new_edges, all_conflicts)

    print(f"Merged grammar files written to: {output_dir.resolve()}")
