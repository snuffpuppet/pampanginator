#!/usr/bin/env python3
"""
package_contribution.py — Package a grammar contributor's export into a zip archive.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Package grammar contribution export files into a zip archive"
    )
    ...
    with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for filename in REQUIRED_FILES:
            if filename == "manifest.json":
                continue
            zf.write(input_dir / filename, arcname=filename)

    print(f"Packaged {nodes_count} grammar nodes, {edges_count} grammar edges")
    print(f"Output: {output_path}")
    print()
    print("Send this file to the maintainer for review and merging.")
    print()
    print("Maintainer instructions:")
    print(f"  1. Unzip: unzip {output_path} -d /tmp/{safe_name}/")
    print(f"  2. Merge: python scripts/merge_contributions.py "
          f"--canonical data/ --contributions /tmp/{safe_name}/ "
