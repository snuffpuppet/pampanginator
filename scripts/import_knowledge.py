#!/usr/bin/env python3
"""
import_knowledge.py — Import canonical data files into the local PostgreSQL database.

Used after pulling updated canonical files from the repository or after a
maintainer publishes an updated canonical file. Supports incremental import
(only new entries) or full reseed (truncate and reimport all).
...
# sentence-transformers is a heavy import — only load when actually embedding
_model = None
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Import canonical knowledge files into PostgreSQL")
    parser.add_argument("--input", default="data/", metavar="DIR",
                        help="Directory containing vocabulary.json, grammar_nodes.json, "
                             "grammar_edges.json (default: data/)")
    ...
    if args.mode == "full" and not args.dry_run:
        answer = input(
            "\nWARNING: Full mode will truncate vocabulary, grammar_nodes, and grammar_edges.\n"
